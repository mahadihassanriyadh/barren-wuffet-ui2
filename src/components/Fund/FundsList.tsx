      ...
      return (
        <>
          <div className="flex items-center justify-between space-x-4">
            ...
            <img
              className={`${
                props.changedPercent || 0 >= 0 ? "bg-[#072213]" : "bg-[#301616]"
              } p-1.5 rounded-full`}
              src={props.changedPercent || 0 >= 0 ? upIcon : downIcon}
              alt=""
            />
          </div>
        </>
    ...
    case "amount":
      ...
      return (
        <>
          <div className="flex items-center justify-between space-x-4">
            <p
              className={`text-sm ${isPos ? "text-green-400" : "text-red-400"}`}
            >
  ...
  }),
  {
    ...
    cell: ({ row }: { row: Row<Fund> }) => {
      ...
      if (status === FundStatus.RAISING) {
        ...
        return <BorderlessButton label="See Investment" />;
      }
    },
  },
];
